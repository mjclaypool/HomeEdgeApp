...
export default function TaskCards( props : cardProps ) {
    return (
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap sm:justify-between gap-6 py-[12px]">
            {props.cardList && props.cardList.map(card => (
                <div key={card.id} className="w-full sm:w-[165px] h-[85px] bg-cc-offw rounded-lg text-cc-prim font-medium">
                    <Link to={card.id} className="flex flex-col justify-between w-full h-full p-[6px]">
                        <h3 className="text-[14px]">{card.name}</h3>
                        <div className="flex justify-between">
                            <h4 className="text-[12px] truncate">{card.nextReminder}</h4>
